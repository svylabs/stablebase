...
interface IStableBaseCDP {
    ...
    function updateReserveRatio(
        ...
        uint256 newPositionInReserve
    ) external;

    function updateOriginationFee(
        address collateralToken,
        uint256 newOriginationFee
    ) external;

    function setStabilityPoolCanReceiveRewards(bool canReceiveRewards) external;

    function setSBRStakingCanReceiveRewards(bool canReceiveRewards) external;
}
