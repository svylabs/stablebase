...
One of the central functions of a reserve bank in traditional finance is to maintain price stability, primarily through controlling interest rates and other tools like the Repo rate and Reserve Ratio, which regulate the money supply. In the realm of cryptocurrency, stablecoin protocols also strive for price stability, typically relying on interest rates and fixed collateral requirements. This paper introduces StableBase, a novel stablecoin protocol utilizing the Collateralized Debt Position (CDP) mechanism, which achieves price stability through two innovative policy tools: **user-set Reserve Ratio** and **pre-paid Shielding Rate**. These tools are designed to dynamically contract and expand the money supply and generate yield without relying on interest rates, enabling the system to quickly adjust to various market conditions. By empowering users to manage these parameters themselves without an elaborate governance mechanism, StableBase ensures price parity with the pegged currency while offering greater flexibility and predictability for a broad user base. StableBase stands out as a compelling alternative to current stablecoin protocols, finding a middle ground between those with 0% interest rates and those with positive interest rates, with the potential to set a new standard in the industry.
