...
One of the central functions of a reserve bank in traditional finance is to maintain price stability, primarily through controlling interest rates and other tools like the Repo rate and Reserve Ratio, which regulate the money supply. In the realm of cryptocurrency, stablecoin protocols also strive for price stability, typically relying on interest rates and fixed collateral requirements. This paper introduces StableBase, a novel stablecoin protocol utilizing the Collateralized Debt Position (CDP) mechanism, which achieves price stability through two innovative policy tools: **user-set Reserve Ratio** and **pre-paid Shielding Rate**, designed to contract and expand money supply and generate yield. By allowing users to set these parameters, the system can dynamically adjust to various market conditions more quickly, ensuring price parity with the pegged currency while offering greater flexibility and predictability for a broad user base. StableBase provides a robust alternative to existing stablecoin protocols, finding a middle ground between those with 0% interest rates and those with positive interest rates, and has the potential to set a new standard in the industry.
