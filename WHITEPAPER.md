                           StableBase: A novel stablecoin protocol with enhanced borrowing experience
                                    Sridhar G<sg@svylabs.com>, Gopalakrishnan G<gopalakrishnan.g@gov.in>
                        (SVY Labs,Netherlands), (PGPPM-IIM Bangalore, Civil Servant, Government of India)

# Abstract

One of the central functions of a reserve bank in traditional finance is to maintain price stability, primarily through controlling interest rates and other tools like the Repo rate and Reserve Ratio, which regulate the money supply. In the realm of cryptocurrency, stablecoin protocols also strive for price stability, typically relying on interest rates and fixed collateral requirements. This paper introduces StableBase, a novel stablecoin protocol utilizing the Collateralized Debt Position (CDP) mechanism. It achieves price stability through two innovative policy tools: **user-set Reserve Ratio** and **pre-paid Shielding Rate**. These tools enable the system to autonomously adjust the money supply as well as generate yield for stablecoin holders without relying on interest rates. By enabling users to manage these parameters without complex governance, StableBase ensures price parity with the pegged currency while offering greater flexibility and predictability for users. StableBase stands out as a compelling alternative to existing stablecoin protocols, striking a balance between protocols offering 0% interest rates and the ones offering positive interest rates, and has the potential to set a new standard in the industry.
